...
class _DashboardViewState extends ConsumerState<DashboardView> {
  ...
  void initState() {
    super.initState();
    LoginState().loadVip = _loadVip;
    LoginState().loadVip();
    if (Platform.isAndroid) Future.delayed(const Duration(seconds: 2), getVersionInfo);
  }
  ...
  void _loadVip() {
    CloudRequest().getSubscribe().then((value) async {
      ...
      setState(() {
        _planName = data?.plan?.name ?? '';
        _expiredAt = getExpireAt(data?.expiredAt);
        _transfer = getTransfer(used);
        _allTransfer = getTransfer(total);
        _progress = progress;

        if (planId == null || expired || transferFull) {
          LoginState().vip = VipState.abnormal;
        } else {
          LoginState().vip = VipState.normal;
        }
      });
      ...
      debugPrint('getSubscribe error: $e');
      setState(() {
        LoginState().vip = VipState.normal;
      });
    });
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    onTap: () async {
                      ...
                      // OtaUpdate().execute(downloadUrl, destinationFilename: 'last.apk').listen((OtaEvent event) {
                      //   if (event.status == OtaStatus.DOWNLOADING) _btnTitle = '已下载${event.value}%';
                      //   if (event.status == OtaStatus.INSTALLING) _btnTitle = '安装中...';
                      //   updateState(() {});
                      //   if (event.status != OtaStatus.DOWNLOADING) Navigator.of(c).pop();
                      // });
                    },
